...
const wallets = { pumpMint: "M", solCreator: "C", evmLauncher: "0xL", evmMaker: "0xK", payment: "P" };
...
test("save/load round-trips and keys are private", () => {
  ...
  const rec = newRecord({ id: "doggo-ab12", token, devWallet: "H6CS", wallets, quote, deadlineAt: 10, now: 5 });
  r.save(rec);
...
test("transition enforces the state machine", () => {
  const rec = newRecord({ id: "a-0000", token, devWallet: "D", wallets, quote, deadlineAt: 10, now: 5 });
...
test("list filters by status, newest first", () => {
  ...
  for (const [id, t] of [["a-0001", 1], ["b-0002", 2]] as const) {
    r.save(newRecord({ id, token, devWallet: "D", wallets, quote, deadlineAt: 10, now: t }));
  }
