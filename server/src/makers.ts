...
export class Makers {
  ...
  arm(coin: CoinState) {
    ...
    const m = new Maker(this.cfg, coin, this.conn, Keypair.fromSecretKey(Uint8Array.from(keys.solMaker ?? keys.solCreator)), this.pub, walletClient(this.cfg.evm.rpcUrl, keys.evmMaker), this.recovery);
