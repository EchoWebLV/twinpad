...
> For EVM OFTs used in this flow, if you need initial tokens on testnet, open the EVM `contracts/LockstepOFT.sol` and uncomment `_mint(msg.sender, 100000 * (10 ** 18));` in the constructor. Ensure you remove this line for production.
...
- (required) for EVM OFTs used in this flow, if you uncommented the testnet mint line in `contracts/LockstepOFT.sol`, ensure it is commented out for production
