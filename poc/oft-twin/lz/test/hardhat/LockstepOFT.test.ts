...
describe('LockstepOFT Test', function () {
    // Constant representing a mock Endpoint ID for testing purposes
    const eidA = 1
    const eidB = 2
    // Declaration of variables to be used in the test suite
    let LockstepOFT: ContractFactory
    ...
    before(async function () {
        ...
        LockstepOFT = await ethers.getContractFactory('LockstepOFTMock')
    ...
    beforeEach(async function () {
        ...
        // Deploying two instances of LockstepOFT contract with different identifiers and linking them to the mock LZEndpoint
        myOFTA = await LockstepOFT.deploy('aOFT', 'aOFT', mockEndpointV2A.address, ownerA.address)
        myOFTB = await LockstepOFT.deploy('bOFT', 'bOFT', mockEndpointV2B.address, ownerB.address)

        // Setting destination endpoints in the LZEndpoint mock for each LockstepOFT instance
        ...
        // Setting each LockstepOFT instance as a peer of the other in the mock LZEndpoint
